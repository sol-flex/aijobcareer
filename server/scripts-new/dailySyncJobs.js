 ...
 *
 * Loops through all companies in database and:
 ...
 *    - Marks removed jobs as deprecated (in DB, not in API)
 * 4. Updates company.lastSyncedAt
 *
 * Usage: node dailySyncJobs.js
 */
...
async function dailySyncJobs() {
    ...
    try {
        ...
        // Get companies - filter to only those with supported platforms
        const companyFilter = process.argv[2]; // e.g. node dailySyncJobs.js "Mistral AI"
        ...
        if (companyFilter) {
            console.log(`Filtering to company: ${companyFilter}`);
        } else {
            console.log(`Filtering to companies with Greenhouse, Lever, or Ashby platforms`);
        }
